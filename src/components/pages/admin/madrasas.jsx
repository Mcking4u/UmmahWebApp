...
function MyMadrasas() {
  ...
  async function fetchData() {
    try {
      const response = await new NetworkHandler().getMadrasasAdmin();
  ...
  const validateForm = () => {
    ...
    if (!isEditing && !formData.assigned_to) {
        errors.assigned_to = "You must select an account";
      }
  ...
  const handleaddMadrasaAdmin = async () => {
    if (!validateForm()) {
      return;
    }
    setLoading(true);
    try {
      await new NetworkHandler().addMadrasaAdmin(formData);
      setOpenDialog(false);
      setFormData({
        madrasa_name: "",
        description: "",
  ...
  const handleeditMadrasaAdmin = async () => {
    if (!validateForm()) {
      return;
    }
    setLoading(true);
    try {
      await new NetworkHandler().editMadrasaAdmin({ ...formData, edit_id: editId });
      setOpenDialog(false);
      setFormData({
        madrasa_name: "",
        description: "",
      ...
      >
                ...
                <Grid item xs={12}>
                  <FormControl fullWidth size="small" sx={{mt:2}}>
                    ...
                    <Select
                      ...
                      onChange={(e) =>
                       {
                        const data = {...formData, assigned_to:e.target.value};
                        setFormData(data)
        ...
        <DialogActions>
          ...
          <Button
            ...
            onClick={isEditing ? handleeditMadrasaAdmin : handleaddMadrasaAdmin}
            disabled={loading}
