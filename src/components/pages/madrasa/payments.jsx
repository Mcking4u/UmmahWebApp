...
const Payments = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const filteredPrograms = initData.programs.filter(
          (program) => program.madrasa === initialMadrasa.id
        );
  ...
  useEffect(() => {
    if (selectedMadrasa && selectedProgram && selectedMonth) {
      const fetchFeesData = async () => {
        ...
        const data = await new NetworkHandler().getFees(
          selectedMadrasa,
          selectedProgram,
          formattedDate
        );
  ...
  const handleMadrasaChange = (event) => {
    ...
    const filteredPrograms = allPrograms.filter(
      (program) => program.madrasa === madrasaId
    );
    setPrograms(filteredPrograms);
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Switch
          ...
          onChange={(event) =>
            handlePaidChange(params.row.id, event.target.checked)
          }
        />
      ),
    },
